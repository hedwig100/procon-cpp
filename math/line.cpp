...
// 無限遠点
template <typename T> constexpr Point<T> INFTY(GEOMETRY_INFTY, GEOMETRY_INFTY);
...
// 半直線
template <typename T> using Ray = Line<T>;
...
Point<T> ll_intersection(const Line<T> &l1, const Line<T> &l2) {
    if (sgn(cross(l1.vec(), l2.vec())) == 0)
        return INFTY<T>; // parallel or partially matched
    return l1.begin + l1.vec() * cross(l2.vec(), l2.begin - l1.begin) /
...
template <typename T>
pair<bool, Point<T>> ss_intersection(const Segment<T> &s1,
                                     const Segment<T> &s2) {
    bool is_intersect =
        (iSP(s2.begin, s1.begin, s1.end) * iSP(s2.end, s1.begin, s1.end) <= 0 &&
         iSP(s1.begin, s2.begin, s2.end) * iSP(s1.end, s2.begin, s2.end) <= 0);
...
// 点pが線分s上の点かどうかを判定する.
template <typename T>
constexpr inline bool ison(const Point<T> &p, const Segment<T> &s) {
    return iSP(p, s.begin, s.end) == 0;
}

// pl_distance
// 点pと直線lの距離を求める.
template <typename T>
long double pl_distance(const Point<T> &p, const Line<T> &l) {
    return abs((long double)cross(l.vec(), p - l.begin) / length(l.vec()));
}
...
long double ps_distance(const Point<T> &p, const Segment<T> &s) {
    if (sgn(dot(s.vec(), p - s.begin)) < 0 ||
        sgn(dot(s.countervec(), p - s.end)) < 0) {
...
long double ss_distance(const Segment<T> &s1, const Segment<T> &s2) {
    ...
    return min({ps_distance(s1.begin, s2), ps_distance(s1.end, s2),
                ps_distance(s2.begin, s1), ps_distance(s2.end, s1)});
}

// proj
// ベクトルpを直線lに射影した点を返す.
Point<long double> proj(const Point<long double> &p,
                        const Line<long double> &l) {
    return l.begin +
           normalize(l.vec()) * (dot(l.vec(), p - l.begin) / length(l.vec()));
}
...
// vertical_bisector
// 点p,qの垂直二等分線を求める.
Line<long double> vertical_bisector(const Point<long double> &p,
                                    const Point<long double> &q) {
