...
// A[ik]なるkの最大値を求める。
template <typename T, T INF> struct LongestIncreasingSequence {

    // dp[i] = 長さi+1であるような増加部分列での最後の要素の最小値,
    // ない場合はINF dp_ind[i] =
    // 長さi+1であるような増加部分列での最後の要素の最小値のインデックス,
    // ない場合は-1 prev[i] =
    // A[i]を含む中で最長の増加部分列でのA[i]の前の要素がA[j]であるときj,
    // 先頭のときは-1
    int n;
    vector<T> A, dp;
    ...
    vector<T> restore_lis() {
        ...
        for (int now_ind = dp_ind[max_len - 1]; now_ind >= 0;
             now_ind     = prev[now_ind])
