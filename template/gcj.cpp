...
using namespace std;
#ifdef LOCAL_
void debug_out() { cerr << "\033[0m" << endl; }
template <typename Head, typename... Tail> void debug_out(Head H, Tail... T) {
    cerr << ' ' << H << ',';
    debug_out(T...);
}
#define debug(...)                                                             \
    cerr << "\033[1;36m" << __func__ << ":L" << __LINE__ << " "                \
         << #__VA_ARGS__ << ":",                                               \
        debug_out(__VA_ARGS__)
#define dump(x)                                                                \
    cerr << __func__ << ":L" << __LINE__ << " " << #x << " = " << (x) << endl
#else
#define debug(...) (void(0))
...
struct GCJ {
  private:
    ...
    void output(int testcase_index) {
        // output
        put(testcase_index, false);
    }
